...
const Chart: React.FC<ChartProps> = ({
  ...
  const getIndicatorValuesAtTime = (time: Time): Record<string, any> => {
    ...
    Object.entries(indicators).forEach(([indicatorId, indicatorData]) => {
      ...
      } else if (indicatorId === 'macd' && indicatorData.macd && indicatorData.signal && indicatorData.histogram && index < indicatorData.macd.length && index < indicatorData.signal.length && index < indicatorData.histogram.length) {
        result[indicatorId] = {
          name: 'MACD',
      ...
      } else if (indicatorId === 'bbands' && indicatorData.upper && indicatorData.middle && indicatorData.lower && index < indicatorData.upper.length && index < indicatorData.middle.length && index < indicatorData.lower.length) {
        result[indicatorId] = {
      ...
      } else if (indicatorId === 'adx' && indicatorData.adx && indicatorData.plusDI && indicatorData.minusDI && index < indicatorData.adx.length && index < indicatorData.plusDI.length && index < indicatorData.minusDI.length) {
        result[indicatorId] = {
          name: 'ADX',
  ...
  return <div className={cn('chart-container relative', className)} style={{
    width,
    height
  }}>
      ...
      {currentPrice && <div className="absolute top-2 right-20 z-10">
      ...
      {hoverData.time && hoverData.ohlc && <div className="absolute top-12 right-2 z-10 bg-trading-bg-dark bg-opacity-95 p-4 rounded-md border border-border shadow-lg max-w-64">
          ...
          {Object.keys(hoverData.indicatorValues).length > 0 && <div className="border-t border-border pt-2">
              <div className="font-medium mb-2 text-xs text-primary">Indicators</div>
              {Object.entries(hoverData.indicatorValues).map(([indicatorId, data]) => <div key={indicatorId} className="mb-2 bg-sidebar-accent/10 rounded-sm p-1.5">
                  <div className="font-medium text-xs text-primary mb-1">{data.name}</div>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                    {indicatorId === 'rsi' && <>
                        <div className="text-muted-foreground text-xs">Value</div>
                        <div className={cn("font-mono text-right text-xs font-medium", data.value > 70 ? "text-trading-down" : data.value < 30 ? "text-trading-up" : "")}>
                    ...
                    {indicatorId === 'macd' && <>
                        ...
                        <div className={cn("font-mono text-right text-xs", Number(data.histogram) > 0 ? "text-trading-up" : "text-trading-down")}>
                    ...
                    {indicatorId === 'adx' && <>
                        ...
                        <div className={cn("font-mono text-right text-xs", Number(data.adx) > 25 ? "text-trading-up font-medium" : "")}>
